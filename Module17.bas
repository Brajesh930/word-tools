...
Set oTable = Selection.Tables(1)
 ...
 For Each claims In claim
      ...
      m = m + 1
      If i > 1 Then
      oTable.cell(charcount - 1, oCell.ColumnIndex - 1).Merge oTable.cell(charcount - i, oCell.ColumnIndex - 1)
      End If
      
      i = 0
     oTable.cell(charcount, oCell.ColumnIndex - 1).Range.Text = "Claim " & Format(m, "00")
     oTable.cell(charcount, oCell.ColumnIndex - 1).Range.Bold = True
     oTable.cell(charcount, oCell.ColumnIndex - 1).Range.ParagraphFormat.Alignment = wdAlignParagraphCenter
     oTable.cell(charcount, oCell.ColumnIndex - 1).Range.Font.Name = "Arial"
     oTable.cell(charcount, oCell.ColumnIndex - 1).Range.Font.Size = 10
     oTable.cell(charcount, oCell.ColumnIndex - 1).Range.Shading.BackgroundPatternColorIndex = wdGray25
     oTable.cell(charcount, oCell.ColumnIndex - 1).Range.ParagraphFormat.SpaceAfter = 6
     oTable.cell(charcount, oCell.ColumnIndex - 1).Range.ParagraphFormat.SpaceBefore = 6
     oTable.cell(charcount, oCell.ColumnIndex - 1).Range.ParagraphFormat.LineSpacing = LinesToPoints(1.5)
     End If
